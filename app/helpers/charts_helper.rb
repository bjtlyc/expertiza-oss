module ChartsHelper
  ...
  def self.get_pie_chart_url(labels, values)
     if check_length(labels,values)
      ...
      address=get_address(address,value_string,label_string,color_string,ifpie)
      #i = 0
      #for value in values
      #  unless value == 0
      #  
      ...
      #cut_length_one(value_string)
      #cut_length_one(label_string)
      #cut_length_one(color_string)
  ...
  def check_length(labels,values)
	  if labels.length != values.length
		  return false
	  else
		  return true
  ...
  def get_address(address,value_string,label_string,color_string,ifpie)
	  i=0
	  if ifpie
		for value in values
        	unless value == 0
	  ...
	  value_string=cut_length_one(value_string)
      	  label_string=cut_length_one(label_string)
      	  color_string=cut_length_one(color_string)	  
      	  
	  address += value_string + label_string + color_string
  end

	  
  def cut_length_one(str)
	  str[0..-2]
  end
